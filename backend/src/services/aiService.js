...
- Flashcards should test definitions, mechanisms, comparisons and exam facts.
- Quiz questions should be multiple choice where possible.
...
async function mergeChunkTopics({
  topics,
  topicName,
  fileName
}) {
  ...
  const mergedRaw = {
    ...
    flashcards: uniqueByQuestion(
      topics.flatMap((topic) => topic.flashcards || [])
    ),
    quizQuestions: uniqueByQuestion(
      topics.flatMap((topic) => topic.quizQuestions || [])
    ),
    glossary: uniqueGlossary(
      topics.flatMap((topic) => topic.glossary || [])
    ),
    sourceFiles: [fileName]
  };

  /**
   * Optional final compression/cleanup pass.
   * To reduce cost, we only call AI again when there are many chunks.
   */
  if (topics.length < 4) {
...
You are merging multiple generated revision chunks into ONE clean topic.
Remove duplicates.
Keep the most exam-relevant flashcards and quiz questions.
  ...
  for (let index = 0; index < textChunks.length; index += 1) {
    if (onProgress) {
      onProgress({
        ...
        message: `Generating revision content from chunk ${index + 1} of ${textChunks.length}`
      });
    }
