...
export default function FlashcardStudySafe({
  ...
  function markCard(isCorrect) {
    ...
    const updatedCard = {
      ...
      incorrectCount: Number(currentCard.incorrectCount || 0) + (isCorrect ? 0 : 1),
      ...
      <section className="revision-glass-card flashcard-study-card">
        <div className="section-heading-row">
          <div>
            ...
            <p className="muted">Try selecting All cards or a different score filter.</p>
          </div>
        </div>

        {onExit && (
          <button type="button" className="revision-btn revision-btn-secondary" onClick={onExit}>
            Back to Topic
          </button>
        )}
      </section>
  ...
  return (
    <section className="revision-glass-card flashcard-study-card">
      ...
      <div className="flashcard-big">
        <p className="flashcard-label">{showAnswer ? "Answer" : "Question"}</p>
      ...
      <div className="button-row spread">
        <button type="button" className="revision-btn revision-btn-secondary" onClick={goBack}>
          ← Previous
        </button>
        ...
        <button type="button" className="revision-btn revision-btn-secondary" onClick={goNext}>
          Next →
        </button>
      </div>
