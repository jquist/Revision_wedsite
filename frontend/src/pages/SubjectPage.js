...
import { normaliseTopic } from "../utils/topicUtils";
...
function SubjectPage({ subject, onBack, onUpdateSubject, currentUserId = "", isDemo = false, readOnly = false }) {
  ...
  function makeUniqueTopic(topic) {
    ...
    const existingIds = new Set((subject.topics || []).map((item) => item.topicId));

    if (!existingIds.has(safeTopic.topicId)) {
      return safeTopic;
    }

    return {
      ...safeTopic,
      topicId: `${safeTopic.topicId}-${Date.now()}`,
    };
  }
  ...
  function handleSaveEditedTopic(updatedTopic) {
  ...
  function handleDeleteEditedTopic(topicId) {
    const remainingTopics = (subject.topics || []).filter((topic) => topic.topicId !== topicId);
    const nextTopicId = remainingTopics[0]?.topicId || ALL_TOPICS_ID;
